...
export class CotisationService {
    ...
    async create(dto: CreateCotisationDto, etablissementId: string, userId?: string, req?: Request): Promise<Cotisation> {
        ...
        if (userId) {
            await auditService.log({ utilisateurId: userId, action: AuditAction.COTISATION_CREATE, cible: 'Cotisation', cibleId: entity.id, description: `Création cotisation ${dto.code}`, nouvellesValeurs: dto, module: 'personnel', etablissementId }, req);
        }
        return entity;
    }
    ...
    async update(id: string, dto: UpdateCotisationDto, etablissementId: string, userId?: string, req?: Request): Promise<Cotisation> {
        ...
        if (userId) {
            await auditService.log({ utilisateurId: userId, action: AuditAction.COTISATION_UPDATE, cible: 'Cotisation', cibleId: id, description: `Mise à jour cotisation ${entity.code}`, anciennesValeurs: oldValues, nouvellesValeurs: dto, module: 'personnel', etablissementId }, req);
        }
        return entity;
    }
    ...
    async delete(id: string, etablissementId: string, userId?: string, req?: Request): Promise<void> {
        ...
        if (userId) {
            await auditService.log({ utilisateurId: userId, action: AuditAction.COTISATION_DELETE, cible: 'Cotisation', cibleId: id, description: `Suppression cotisation ${entity.code}`, module: 'personnel', etablissementId }, req);
        }
    }
}
