...
export class OrganisationService {
    ...
    async createPoste(dto: CreatePosteDto): Promise<Poste> {
        // Vérifier que l'unité existe
        const unite = await this.uniteRepo.findOne({ where: { id: dto.uniteOrganisationnelleId } });
        if (!unite) {
            throw new AppError('Unité organisationnelle non trouvée', 404, 'UNITE_NOT_FOUND');
        }

        // Vérifier l'unicité du code au sein de l'unité
        ...
        const poste = this.posteRepo.create({
            ...
            superviseurNom: dto.superviseurNom,
    ...
    async findPostes(filtres: FiltrePostesDto, etablissementId?: string): Promise<Poste[]> {
        const where: any = {};

        if (filtres.uniteOrganisationnelleId) {
            where.uniteOrganisationnelleId = filtres.uniteOrganisationnelleId;
        }
    ...
    async findPosteById(id: string, etablissementId?: string): Promise<Poste> {
        let poste: Poste | null;

        if (etablissementId) {
            poste = await this.posteRepo.findOne({
                where: { id, uniteOrganisationnelle: { organisation: { etablissementId } } },
                relations: ['uniteOrganisationnelle'],
            });
        } else {
            poste = await this.posteRepo.findOne({
    ...
    async updatePoste(id: string, dto: UpdatePosteDto): Promise<Poste> {
        const poste = await this.findPosteById(id);
        const uniteOrganisationnelleId = poste.uniteOrganisationnelleId;
        
        Object.assign(poste, dto);

        // Mise à jour intelligente du statut selon l'occupant
        if (dto.occupantId !== undefined) {
            // Si on fournit explicitement un occupantId (même null)
            poste.statut = dto.occupantId ? StatutPoste.ACTIF : StatutPoste.VACANT;
        } else if (!poste.occupantId) {
            // Si pas d'occupant dans le DTO et pas d'occupant existant
            poste.statut = StatutPoste.VACANT;
        }
        // Sinon: on garde le statut existant

        const updated = await this.posteRepo.save(poste);
        logger.info(`Poste modifié: ${updated.intitulé}`, { posteId: updated.id });

        // Invalider le cache de l'organisation
        const unite = await this.uniteRepo.findOne({ where: { id: uniteOrganisationnelleId } });
        if (unite) {
            await this.invalidateArborescenceCache(unite.organisationId);
        }
    ...
    async assignerOccupant(posteId: string, occupantId: string, occupantNom: string): Promise<Poste> {
        const poste = await this.findPosteById(posteId);

        poste.occupantId = occupantId;
        poste.occupantNom = occupantNom;
        poste.statut = StatutPoste.ACTIF;

        const updated = await this.posteRepo.save(poste);
        logger.info(`Occupant assigné au poste: ${poste.intitulé}`, { posteId, occupantId });
        return updated;
    }
    ...
    async libererPoste(posteId: string): Promise<Poste> {
        ...
        poste.occupantId = undefined;
        poste.occupantNom = undefined;
        poste.statut = StatutPoste.VACANT;

        const updated = await this.posteRepo.save(poste);
        logger.info(`Poste libéré: ${poste.intitulé}`, { posteId });
        return updated;
    }
