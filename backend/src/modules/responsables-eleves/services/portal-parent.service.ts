...
export class PortalParentService {
    ...
    async getEnfantsParent(parentId: string): Promise<any[]> {
        const responsabilites = await parentsService.getEnfantsParent(parentId);
    ...
    async getNotesEnfant(parentId: string, enfantId: string, filters?: {
        ...
        // Vérifier l'accès
        const peutAcceder = await parentsService.peutAccederEleve(parentId, enfantId);
        if (!peutAcceder) {
    ...
    async getPaiementsEnfant(parentId: string, enfantId: string): Promise<any> {
        const peutPayer = await parentsService.peutPayerPourEleve(parentId, enfantId);
        if (!peutPayer) {
