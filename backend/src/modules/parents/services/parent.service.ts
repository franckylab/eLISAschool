/**
 * ==================================
 * eLISAschool - Service Parents (Module Parents)
 * ==================================
 * Version: 3.0.0
 * Auteur: franck arlos chendjou
 * 
 * Description: Service pour gérer les relations entre parents et élèves.
 * Module payant — activable/désactivable sans impact sur le reste du système.
 */

import { Repository, IsNull } from 'typeorm';
import { AppDataSource } from '@database/data-source';
import { Parent, LienParente } from '../entities';
...
export class ParentService {
    private responsableRepo: Repository<Parent>;
    private utilisateurRepo: Repository<Utilisateur>;
    private eleveRepo: Repository<Eleve>;

    constructor() {
        this.responsableRepo = AppDataSource.getRepository(Parent);
     ...
     */
    async lierParent(dto: LierParentDto, req?: Request): Promise<Parent> {
     ...
     */
    async getParentsEleve(enfantUtilisateurId: string): Promise<Parent[]> {
     ...
     */
    async getEnfantsParent(parentUtilisateurId: string): Promise<Parent[]> {
        return this.responsableRepo.find({
            where: {
     ...
     * 
     * PRIORITÉ 1 : Parent (source de vérité pour inscriptions)
    ...
    async getParentsInfo(eleveId: string): Promise<Array<{
        ...
        // PRIORITÉ 1 : Essayer Parent (source de vérité)
        ...
        if (responsables.length > 0) {
            logger.debug(`[ParentsInfo] ${responsables.length} parent(s) trouvé(s) via Parent pour élève ${eleveId}`);
     ...
     */
    async findOne(parentId: string, enfantId: string): Promise<Parent> {
        const responsable = await this.responsableRepo.findOne({
            where: {
                utilisateurId: parentId,
                enfantId,
    ...
    /**
     * Migrer les champs directs d'un élève vers Parent
     * 
        ...
        for (const parentData of parentsData) {
            try {
                ...
                if (utilisateurParent) {
                    ...
                    if (!existing) {
                        logger.info(`[Migration] Création lien Parent pour ${parentData.lienParente}`);
