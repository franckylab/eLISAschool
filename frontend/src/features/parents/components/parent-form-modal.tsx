/**
 * ==================================
 * eLISAschool - Modal Formulaire Parent (Module Parents)
 * ==================================
 * Version: 2.0.0
...
import type { Parent, CreerParentDto } from '../types/parent.types';

interface ParentFormModalProps {
    open: boolean;
    parent: Parent | null;
    onClose: () => void;
    onSubmit: (dto: CreerParentDto) => Promise<void>;
}
...
export function ParentFormModal({
    ...
    const [utilisateurId, setUtilisateurId] = useState(parent?.utilisateurId || '');
    const [enfantId, setEnfantId] = useState(parent?.enfantId || '');
    const [lienParente, setLienParente] = useState(parent?.lienParente || 'PERE');
    const [responsableLegal, setResponsableLegal] = useState(parent?.responsableLegal ?? true);
    ...
    const validate = (): boolean => {
        ...
        if (!utilisateurId) {
            newErrors.utilisateurId = 'Le parent est requis';
        }
    ...
    return (
        <CustomModal
            ...
            title={isEditMode ? 'Modifier le parent' : 'Nouveau parent'}
            description="Remplissez les informations du parent"
            size="2xl"
            footer={<>
