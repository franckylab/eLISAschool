...
import { useParentDetail } from '../hooks/use-parents';
...
export function ParentDetailPage() {
    const { id } = useParams({ from: '/_auth/parents/$id' });
    const navigate = useNavigate();
    const { t } = useTranslation('parents');
    const { data: parent, isLoading, error } = useParentDetail(id);
            ...
            <TabsContent activeTab={ongletActif}>
                {ongletActif === 'informations' && (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <Card>
                            <CardHeader>
                                <CardTitle>{t('detailsParent')}</CardTitle>
                            </CardHeader>
