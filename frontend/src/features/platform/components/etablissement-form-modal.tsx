...
// Refonte v3 — plan piloté par JSONB (quotas libres, cycles autorisés)
...
export function EtablissementFormModal({ open, onOpenChange, etablissement, mode }: EtablissementFormModalProps) {
    ...
    // Fetch plans actifs (create uniquement) — enveloppe { success, data }
    const { data: plans } = useQuery<PlanOption[] | undefined>({
        queryKey: ['platform-plans-options'],
        queryFn: async () => {
            const res = await apiClient.get<PlanOption[] | { success: boolean; data: PlanOption[] }>('/api/platform/facturation/plans');
            const payload = res.data as any;
            const liste: PlanOption[] = Array.isArray(payload) ? payload : payload?.data ?? [];
            return liste
        ...
        <div className="space-y-4">
                    ...
                    >
                        <div>
                            <div className="font-medium flex items-center gap-2">
                                {plan.nom}
                                {plan.estParDefaut && (
                                    <span className="text-xs px-2 py-0.5 rounded-full" style={{ backgroundColor: 'color-mix(in srgb, var(--color-dominant-600) 12%, transparent)', color: 'var(--color-dominant-600)' }}>
                                        {t('etablissementForm.plan.parDefaut', 'Par défaut')}
                                    </span>
                                )}
                            </div>
                            <div className="text-sm" style={{ color: 'var(--color-texte-muted)' }}>
                                {(plan.quotas?.eleves ?? 0) === 0
                                    ? t('etablissementForm.plan.elevesIllimites', 'Élèves illimités')
                                    : t('etablissementForm.plan.maxEleves', { max: plan.quotas?.eleves })}
                            </div>
                        </div>
            ...
            {form.planId && (
                <div>
                    ...
                    >
                        {CYCLES_FACTURATION
                            .filter(c => !selectedPlan?.cyclesAutorises?.length || selectedPlan.cyclesAutorises.includes(c.value))
