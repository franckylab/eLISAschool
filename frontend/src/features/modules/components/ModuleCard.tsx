/**
 * ModuleCard — Carte d'un module dans le catalogue
...
const CATEGORIE_STYLES: Record<string, string> = {
    CRITIQUE: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20',
...
export function ModuleCard({
    ...
    const isCritique = module.category === 'CRITIQUE';
        ...
        >
            ...
            <div className="flex items-start justify-between gap-3">
                ...
                {onToggle && (
                    <button
                        type="button"
                        disabled={isToggling || isCritique || isLocked}
                        onClick={(e) => {
                            e.stopPropagation();
                        ...
                        className={cn(
                            ...
                            (isToggling || isCritique || isLocked) && 'opacity-50 cursor-not-allowed',
                        )}
                        title={isLocked ? (module.messageBlocage || 'Module verrouillé') : isCritique ? 'Module critique (non désactivable)' : actif ? 'Désactiver' : 'Activer'}
                    >
                        <span
                            className={cn(
