 ...
 * CRUD, membres, configuration modules/tranches/abonnement.
 * 
...
function ConfigureGroupeModal({
    ...
    const { t } = useTranslation('admin');
    const [activeTab, setActiveTab] = useState<'membres' | 'modules' | 'tranches' | 'abonnement'>('membres');
    ...
    const tabs = [
        ...
        { id: 'tranches' as const, label: t('groupes.tabs.tranches', 'Tranches'), icon: Layers },
        ...
        >
            <div className="flex flex-col gap-[var(--gap-md)]">
                ...
                <div className="min-h-[300px]">
                    ...
                    {activeTab === 'tranches' && <TranchesTab groupe={groupe} />}
...
function TranchesTab({ groupe }: { groupe: GroupeEtablissement }) {
    const { t } = useTranslation('admin');
    const { data: tranches, isLoading } = useQuery({
        queryKey: ['groupes-tranches', groupe.id],
        queryFn: async () => {
            const res = await apiClient.get<any[]>(`/api/platform/facturation/groupes/${groupe.id}/tranches`);
            return res.data ?? [];
        },
    });
    ...
    return (
        <div className="flex flex-col gap-[var(--gap-md)]">
            {!tranches?.length ? (
                <p className="text-center text-[var(--text-sm)] text-[var(--color-text-tertiary)] py-[var(--space-lg)]">
                    {t('groupes.tranchesComing', 'Aucune tranche configurée pour ce groupe')}
                </p>
            ) : (
                <div className="border rounded-[var(--radius-lg)] overflow-hidden">
                    <table className="w-full text-sm">
                        <thead className="bg-[var(--color-surface-hover)]">
                            <tr>
                                <th className="text-left p-[var(--space-sm)] font-medium">{t('groupes.tranches.label', 'Label')}</th>
                                <th className="text-right p-[var(--space-sm)] font-medium">{t('groupes.tranches.min', 'Min')}</th>
                                <th className="text-right p-[var(--space-sm)] font-medium">{t('groupes.tranches.max', 'Max')}</th>
                                <th className="text-right p-[var(--space-sm)] font-medium">{t('groupes.tranches.montant', 'Montant')}</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-[var(--color-bordure)]">
                            {tranches.map((tr: any, i: number) => (
                                <tr key={tr.id || i}>
                                    <td className="p-[var(--space-sm)]">{tr.label || `Tranche ${i + 1}`}</td>
                                    <td className="p-[var(--space-sm)] text-right font-mono">{tr.minEleves}</td>
                                    <td className="p-[var(--space-sm)] text-right font-mono">{tr.maxEleves ?? '∞'}</td>
                                    <td className="p-[var(--space-sm)] text-right font-mono">{tr.montantSupplementaire} XAF</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
