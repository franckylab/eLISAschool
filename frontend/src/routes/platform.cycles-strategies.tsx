/**
 * ==================================
 * eLISAschool - Plateforme — Cycles & Stratégies d'expiration
 * ==================================
 *
 * CRUD des cycles de facturation configurables et des stratégies
 * d'expiration d'abonnement (Refonte v3, migration 213).
 *   - Cycles : durée en mois, remise associée (annuel −10 %…)
 *   - Stratégies : phases ordonnées de dégradation gracieuse
 *     (READ_ONLY → LOCKED → ARCHIVED)
 *
 * Refonte v4.3 — Types partagés + modals CustomModal + i18n
 *
 * Version: 4.0.0
...
function useStrategies() {
    return useQuery<StrategieExpiration[]>({
        ...
        queryFn: async () => {
            const res = await apiClient.get<{ success: boolean; data: StrategieExpiration[] }>('/api/platform/strategies-expiration');
            return res.data?.data ?? [];
        },
    });
}
...
function CyclesStrategiesPage() {
    ...
    const handleDeleteCycle = (cycle: CycleFacturation) => {
        setCycleToDelete(cycle);
    };

    const handleDeleteStrategie = (strategie: StrategieExpiration) => {
        setStrategieToDelete(strategie);
    };
        ...
        <div className="space-y-[var(--space-lg)] p-[clamp(1rem,0.75rem+1vw,1.5rem)]">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <CalendarClock className="h-6 w-6 text-[var(--color-dominante)]" />
                    <div>
            ...
            {tab === 'cycles' && (
                <div className="overflow-x-auto rounded-2xl border border-[var(--color-bordure)] bg-[var(--color-surface)]">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {cyclesLoading && (
                                <tr><td colSpan={6} className="px-4 py-8 text-center text-[var(--color-texte-secondaire)]">{t('cycles.chargement')}</td></tr>
                            )}
                            {(cycles ?? []).map(c => (
                                <tr key={c.id} className="border-b border-[var(--color-bordure)]/50 text-[var(--color-texte)] transition-colors hover:bg-[var(--color-surface-hover)]/50">
                            ...
                            {!cyclesLoading && (cycles ?? []).length === 0 && (
                                <tr><td colSpan={6} className="px-4 py-12 text-center">
                                    <div className="flex flex-col items-center">
                                        <CalendarClock className="mb-3 h-10 w-10 text-[var(--color-texte-muted)]" />
            ...
            {tab === 'strategies' && (
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                    {strategiesLoading && (
                        <div className="col-span-full py-8 text-center text-[var(--color-texte-secondaire)]">{t('cycles.chargement')}</div>
                    )}
                    {(strategies ?? []).map(s => (
                        <div key={s.id} className="flex flex-col rounded-2xl border border-[var(--color-bordure)] bg-[var(--color-surface)] p-5">
                    ...
                    {!strategiesLoading && (strategies ?? []).length === 0 && (
                        <div className="col-span-full flex flex-col items-center rounded-2xl border-2 border-dashed border-[var(--color-bordure)] py-12">
                            <Hourglass className="mb-3 h-10 w-10 text-[var(--color-texte-muted)]" />
            ...
            {/* Confirmation suppression stratégie */}
            <ConfirmationModal
