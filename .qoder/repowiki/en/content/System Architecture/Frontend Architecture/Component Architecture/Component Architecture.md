# Component Architecture

<cite>
**Referenced Files in This Document**
- [App.tsx](file://frontend/src/App.tsx)
- [main.tsx](file://frontend/src/main.tsx)
- [routeTree.gen.ts](file://frontend/src/routeTree.gen.ts)
- [components/ui/index.ts](file://frontend/src/components/ui/index.ts)
- [components/ui/DataTable.tsx](file://frontend/src/components/ui/DataTable.tsx)
- [components/ui/Card.tsx](file://frontend/src/components/ui/Card.tsx)
- [components/ui/Button.tsx](file://frontend/src/components/ui/Button.tsx)
- [features/auth/AuthProvider.tsx](file://frontend/src/features/auth/AuthProvider.tsx)
- [features/dashboard/DashboardProvider.tsx](file://frontend/src/features/dashboard/DashboardProvider.tsx)
- [hooks/useAuth.ts](file://frontend/src/hooks/useAuth.ts)
- [hooks/usePagination.ts](file://frontend/src/hooks/usePagination.ts)
- [lib/api.ts](file://frontend/src/lib/api.ts)
- [stores/appStore.ts](file://frontend/src/stores/appStore.ts)
</cite>

## Update Summary
**Changes Made**
- Enhanced documentation for organizational chart visualization system with improved drag-and-drop functionality
- Updated component architecture details for redesigned node components in organigramme system
- Expanded interactive features documentation including advanced manipulation capabilities
- Added performance optimizations for complex chart rendering and user interactions
- Updated state management patterns for enhanced chart data handling

## Table of Contents
1. [Introduction](#introduction)
2. [Project Structure](#project-structure)
3. [Core Components](#core-components)
4. [Architecture Overview](#architecture-overview)
5. [Interactive Organizational Chart System](#interactive-organizational-chart-system)
6. [Enhanced Nomenclature Management](#enhanced-nomenclature-management)
7. [Template Generation System](#template-generation-system)
8. [Consolidated Navigation Structure](#consolidated-navigation-structure)
9. [Detailed Component Analysis](#detailed-component-analysis)
10. [Dependency Analysis](#dependency-analysis)
11. [Performance Considerations](#performance-considerations)
12. [Troubleshooting Guide](#troubleshooting-guide)
13. [Conclusion](#conclusion)

## Introduction
This document explains the React component architecture with a focus on feature-based organization, shared UI components, composition patterns, prop interfaces, event handling, providers for global state and context, testing strategies, and performance optimization techniques. The architecture has been significantly enhanced with new interactive visualization systems, advanced management interfaces, and streamlined navigation structures to support complex organizational data management and template-driven workflows. Recent improvements include substantial enhancements to the organizational chart visualization system with advanced drag-and-drop capabilities and redesigned node components.

## Project Structure
The frontend follows a feature-based organization where each business domain owns its components, hooks, types, and utilities. Shared UI primitives live under a common UI layer. Providers encapsulate global state and contexts. Routes are generated by the router and compose feature pages. The recent overhaul introduces specialized modules for organizational charting, nomenclature management, and template generation with enhanced interactive capabilities.

```mermaid
graph TB
subgraph "Entry"
main_tsx["main.tsx"]
app_tsx["App.tsx"]
route_tree["routeTree.gen.ts"]
end
subgraph "Shared UI"
ui_index["components/ui/index.ts"]
ui_datatable["components/ui/DataTable.tsx"]
ui_card["components/ui/Card.tsx"]
ui_button["components/ui/Button.tsx"]
ui_chart["components/ui/OrganisationalChart.tsx"]
ui_template["components/ui/TemplateGenerator.tsx"]
ui_node["components/ui/ChartNode.tsx"]
ui_connector["components/ui/ChartConnector.tsx"]
end
subgraph "Features"
auth_provider["features/auth/AuthProvider.tsx"]
dashboard_provider["features/dashboard/DashboardProvider.tsx"]
org_chart["features/organisation/OrganisationalChart.tsx"]
nomenclature["features/nomenclature/NomenclatureManager.tsx"]
templates["features/templates/TemplateSystem.tsx"]
end
subgraph "Hooks"
use_auth["hooks/useAuth.ts"]
use_pagination["hooks/usePagination.ts"]
use_chart["hooks/useOrganisationalChart.ts"]
use_templates["hooks/useTemplates.ts"]
end
subgraph "Libraries"
api_lib["lib/api.ts"]
chart_engine["lib/chartEngine.ts"]
template_engine["lib/templateEngine.ts"]
drag_drop["lib/dragDropEngine.ts"]
end
subgraph "Stores"
app_store["stores/appStore.ts"]
chart_store["stores/chartStore.ts"]
template_store["stores/templateStore.ts"]
end
main_tsx --> app_tsx
app_tsx --> route_tree
route_tree --> auth_provider
route_tree --> dashboard_provider
route_tree --> org_chart
route_tree --> nomenclature
route_tree --> templates
auth_provider --> use_auth
dashboard_provider --> use_pagination
org_chart --> use_chart
nomenclature --> use_templates
ui_datatable --> use_pagination
ui_card --> ui_button
ui_chart --> use_chart
ui_template --> use_templates
ui_node --> drag_drop
ui_connector --> drag_drop
ui_button --> api_lib
app_tsx --> app_store
```

**Diagram sources**
- [main.tsx](file://frontend/src/main.tsx)
- [App.tsx](file://frontend/src/App.tsx)
- [routeTree.gen.ts](file://frontend/src/routeTree.gen.ts)
- [components/ui/index.ts](file://frontend/src/components/ui/index.ts)
- [components/ui/DataTable.tsx](file://frontend/src/components/ui/DataTable.tsx)
- [components/ui/Card.tsx](file://frontend/src/components/ui/Card.tsx)
- [components/ui/Button.tsx](file://frontend/src/components/ui/Button.tsx)
- [features/auth/AuthProvider.tsx](file://frontend/src/features/auth/AuthProvider.tsx)
- [features/dashboard/DashboardProvider.tsx](file://frontend/src/features/dashboard/DashboardProvider.tsx)
- [hooks/useAuth.ts](file://frontend/src/hooks/useAuth.ts)
- [hooks/usePagination.ts](file://frontend/src/hooks/usePagination.ts)
- [lib/api.ts](file://frontend/src/lib/api.ts)
- [stores/appStore.ts](file://frontend/src/stores/appStore.ts)

**Section sources**
- [main.tsx](file://frontend/src/main.tsx)
- [App.tsx](file://frontend/src/App.tsx)
- [routeTree.gen.ts](file://frontend/src/routeTree.gen.ts)

## Core Components
- Feature-based domains: Each feature folder contains its own components, hooks, types, and utilities. This isolates concerns and improves maintainability.
- Shared UI layer: Reusable primitives (e.g., DataTable, Card, Button) are centralized under components/ui and re-exported via an index file for consistent imports.
- Composition patterns: Complex screens are composed from small, focused components. Props define clear contracts; events are passed as callbacks.
- Providers: Global state and contexts are provided at appropriate boundaries (e.g., authentication, dashboard configuration).
- Hooks: Domain-specific logic is extracted into custom hooks (e.g., useAuth, usePagination), promoting reuse and testability.
- API integration: Network calls are abstracted through a library module to keep components free of HTTP details.
- **Enhanced**: Advanced visualization components with sophisticated drag-and-drop functionality and redesigned node components for better user interaction and performance.

**Section sources**
- [components/ui/index.ts](file://frontend/src/components/ui/index.ts)
- [components/ui/DataTable.tsx](file://frontend/src/components/ui/DataTable.tsx)
- [components/ui/Card.tsx](file://frontend/src/components/ui/Card.tsx)
- [components/ui/Button.tsx](file://frontend/src/components/ui/Button.tsx)
- [features/auth/AuthProvider.tsx](file://frontend/src/features/auth/AuthProvider.tsx)
- [features/dashboard/DashboardProvider.tsx](file://frontend/src/features/dashboard/DashboardProvider.tsx)
- [hooks/useAuth.ts](file://frontend/src/hooks/useAuth.ts)
- [hooks/usePagination.ts](file://frontend/src/hooks/usePagination.ts)
- [lib/api.ts](file://frontend/src/lib/api.ts)

## Architecture Overview
The application bootstraps via the entry point, mounts the root App, and uses a generated route tree to render feature routes. Providers wrap relevant sections to supply global state and contexts. Shared UI components are used across features to ensure consistency. The enhanced architecture now supports complex interactive visualizations and template-driven workflows through specialized providers and stores, with significant improvements to the organizational chart system's interactivity and performance.

```mermaid
sequenceDiagram
participant Entry as "main.tsx"
participant Root as "App.tsx"
participant Router as "routeTree.gen.ts"
participant AuthP as "AuthProvider.tsx"
participant DashP as "DashboardProvider.tsx"
participant OrgChartP as "OrganisationalChartProvider.tsx"
participant TemplateP as "TemplateProvider.tsx"
participant Page as "Feature Page"
participant UI as "UI Components"
participant Hook as "Custom Hooks"
participant API as "lib/api.ts"
participant ChartEngine as "chartEngine.ts"
participant DragDrop as "dragDropEngine.ts"
participant TemplateEngine as "templateEngine.ts"
Entry->>Root : "Mount React tree"
Root->>Router : "Render route tree"
Router->>AuthP : "Wrap authenticated routes"
Router->>DashP : "Wrap dashboard routes"
Router->>OrgChartP : "Wrap organisational chart routes"
Router->>TemplateP : "Wrap template routes"
AuthP-->>Page : "Provide auth context"
DashP-->>Page : "Provide dashboard context"
OrgChartP-->>Page : "Provide chart context"
TemplateP-->>Page : "Provide template context"
Page->>Hook : "useAuth(), usePagination()"
Page->>ChartHook : "useOrganisationalChart()"
Page->>TemplateHook : "useTemplates()"
Hook->>API : "Fetch data"
ChartHook->>ChartEngine : "Process chart data"
ChartHook->>DragDrop : "Handle drag operations"
TemplateHook->>TemplateEngine : "Generate templates"
API-->>Hook : "Return data/state"
ChartEngine-->>ChartHook : "Return chart configuration"
DragDrop-->>ChartHook : "Return drag state"
TemplateEngine-->>TemplateHook : "Return template output"
Hook-->>Page : "Expose state/actions"
ChartHook-->>Page : "Expose chart state/actions"
TemplateHook-->>Page : "Expose template state/actions"
Page->>UI : "Render DataTable/Card/Button/Chart/Template"
UI-->>Page : "Emit events (onClick, onChange)"
```

**Diagram sources**
- [main.tsx](file://frontend/src/main.tsx)
- [App.tsx](file://frontend/src/App.tsx)
- [routeTree.gen.ts](file://frontend/src/routeTree.gen.ts)
- [features/auth/AuthProvider.tsx](file://frontend/src/features/auth/AuthProvider.tsx)
- [features/dashboard/DashboardProvider.tsx](file://frontend/src/features/dashboard/DashboardProvider.tsx)
- [hooks/useAuth.ts](file://frontend/src/hooks/useAuth.ts)
- [hooks/usePagination.ts](file://frontend/src/hooks/usePagination.ts)
- [lib/api.ts](file://frontend/src/lib/api.ts)

## Interactive Organizational Chart System

### Overview
The organizational chart system provides multiple visualization modes for displaying hierarchical organizational structures with significantly enhanced drag-and-drop functionality and redesigned node components. The system now supports advanced interactive features including smooth dragging, intelligent positioning, real-time collaboration, and optimized performance for large datasets.

### Enhanced Features
- **Advanced Drag-and-Drop**: Improved drag mechanics with snap-to-grid, collision detection, and smooth animations
- **Redesigned Node Components**: Modernized node appearance with better visual hierarchy and responsive design
- **Multi-Mode Visualization**: Support for tree diagrams, mind maps, flowcharts, network graphs, and timeline views
- **Real-time Collaboration**: Multiple users can interact with charts simultaneously with conflict resolution
- **Performance Optimizations**: Canvas-based rendering for large datasets and efficient memory management

### Key Components
- **OrganisationalChart**: Main container component with enhanced drag-and-drop capabilities
- **ChartNode**: Redesigned node component with improved visual feedback and interaction states
- **ChartConnector**: Enhanced connector system with animated transitions and custom styling
- **ChartControls**: Comprehensive toolbar with zoom, pan, and mode switching
- **ChartLegend**: Dynamic legend with filtering and search capabilities

### Enhanced Drag-and-Drop Functionality
The drag-and-drop system now includes:
- **Intelligent Positioning**: Automatic alignment and spacing during drag operations
- **Collision Detection**: Prevents overlapping nodes and provides visual feedback
- **Snap-to-Grid**: Optional grid snapping for precise positioning
- **Multi-Selection**: Drag multiple nodes simultaneously with relative positioning
- **Undo/Redo**: Full history support for drag operations
- **Touch Support**: Mobile-friendly touch gestures for drag operations

```mermaid
classDiagram
class OrganisationalChart {
+props data
+props mode
+props config
+events onNodeClick
+events onNodeDrag
+events onNodeDrop
+events onModeChange
+methods zoomIn()
+methods zoomOut()
+methods resetView()
+methods enableDrag()
+methods disableDrag()
}
class ChartNode {
+props id
+props label
+props type
+props position
+props metadata
+props draggable
+events onClick
+events onDragStart
+events onDragEnd
+events onContextMenu
+methods updatePosition()
+methods setVisualState()
}
class ChartConnector {
+props source
+props target
+props style
+props animated
+events onClick
+methods animateTransition()
+methods updatePath()
}
class DragDropEngine {
+props chartInstance
+props options
+events onDragStart
+events onDragMove
+events onDragEnd
+methods handleDrag()
+methods detectCollision()
+methods calculatePosition()
}
class ChartControls {
+props currentMode
+props showLegend
+props showGrid
+events onModeChange
+events onToggleLegend
+events onToggleGrid
}
class ChartLegend {
+props items
+props visible
+events onItemSelect
}
OrganisationalChart --> ChartNode : "renders"
OrganisationalChart --> ChartConnector : "connects"
OrganisationalChart --> ChartControls : "controls"
OrganisationalChart --> ChartLegend : "displays"
OrganisationalChart --> DragDropEngine : "uses"
ChartNode --> DragDropEngine : "draggable by"
ChartConnector --> ChartNode : "connected to"
```

**Diagram sources**
- [features/organisation/OrganisationalChart.tsx](file://frontend/src/features/organisation/OrganisationalChart.tsx)
- [components/ui/ChartNode.tsx](file://frontend/src/components/ui/ChartNode.tsx)
- [components/ui/ChartConnector.tsx](file://frontend/src/components/ui/ChartConnector.tsx)
- [components/ui/ChartControls.tsx](file://frontend/src/components/ui/ChartControls.tsx)
- [components/ui/ChartLegend.tsx](file://frontend/src/components/ui/ChartLegend.tsx)
- [lib/dragDropEngine.ts](file://frontend/src/lib/dragDropEngine.ts)

### State Management
The chart system uses a dedicated store for managing complex chart state, including node positions, connections, viewport settings, and user interactions. Custom hooks provide reactive access to chart data and actions with enhanced performance through memoization and selective subscriptions.

**Section sources**
- [features/organisation/OrganisationalChart.tsx](file://frontend/src/features/organisation/OrganisationalChart.tsx)
- [hooks/useOrganisationalChart.ts](file://frontend/src/hooks/useOrganisationalChart.ts)
- [stores/chartStore.ts](file://frontend/src/stores/chartStore.ts)
- [lib/chartEngine.ts](file://frontend/src/lib/chartEngine.ts)
- [lib/dragDropEngine.ts](file://frontend/src/lib/dragDropEngine.ts)

## Enhanced Nomenclature Management

### Overview
The nomenclature management system provides comprehensive tools for managing terminology, definitions, and classification hierarchies. It includes advanced search capabilities, bulk operations, version control, and collaborative editing features.

### Key Features
- **Hierarchical Classification**: Multi-level taxonomy management
- **Advanced Search**: Full-text search with filters and faceted navigation
- **Version Control**: Track changes and rollback capabilities
- **Bulk Operations**: Mass import/export and batch updates
- **Collaborative Editing**: Real-time co-editing with conflict resolution
- **Audit Trail**: Complete change history and compliance tracking

### Component Architecture
- **NomenclatureManager**: Main orchestration component
- **ClassificationTree**: Hierarchical tree view with drag-and-drop
- **SearchInterface**: Advanced search with filters and suggestions
- **EditorPanel**: Rich text editor with validation and preview
- **VersionHistory**: Timeline view of changes with comparison tools

```mermaid
flowchart TD
Start(["User Action"]) --> Search["Search Interface"]
Start --> Tree["Classification Tree"]
Start --> Editor["Editor Panel"]
Search --> Results["Filtered Results"]
Tree --> NodeSelection["Node Selection"]
Editor --> Validation["Content Validation"]
Results --> Preview["Preview Mode"]
NodeSelection --> EditAction["Edit Actions"]
Validation --> SaveDraft["Save Draft"]
Preview --> Review["Review Workflow"]
EditAction --> Update["Update Data"]
SaveDraft --> VersionControl["Version Control"]
Review --> Approve["Approval Process"]
Update --> Sync["Real-time Sync"]
VersionControl --> History["History Tracking"]
Approve --> Publish["Publish Changes"]
Sync --> Collaborate["Collaborative Editing"]
History --> Audit["Audit Trail"]
Publish --> End(["Changes Applied"])
Collaborate --> End
Audit --> End
```

**Section sources**
- [features/nomenclature/NomenclatureManager.tsx](file://frontend/src/features/nomenclature/NomenclatureManager.tsx)
- [features/nomenclature/ClassificationTree.tsx](file://frontend/src/features/nomenclature/ClassificationTree.tsx)
- [features/nomenclature/SearchInterface.tsx](file://frontend/src/features/nomenclature/SearchInterface.tsx)
- [features/nomenclature/EditorPanel.tsx](file://frontend/src/features/nomenclature/EditorPanel.tsx)
- [features/nomenclature/VersionHistory.tsx](file://frontend/src/features/nomenclature/VersionHistory.tsx)

## Template Generation System

### Overview
The template generation system enables dynamic content creation through reusable templates with variable substitution, conditional logic, and multi-format output support. It supports document generation, email templates, reports, and automated communications.

### Core Components
- **TemplateEngine**: Central processing engine for template compilation and execution
- **TemplateDesigner**: Visual template builder with drag-and-drop interface
- **VariableManager**: Variable definition and scope management
- **OutputRenderer**: Multi-format rendering (PDF, HTML, DOCX, etc.)
- **ScheduleManager**: Automated template execution scheduling

### Template Types
- **Document Templates**: Letters, certificates, official documents
- **Email Templates**: Automated notifications and communications
- **Report Templates**: Statistical reports and analytics summaries
- **Form Templates**: Dynamic form generation and validation
- **Notification Templates**: System alerts and user notifications

```mermaid
graph LR
subgraph "Template Input"
A[Template Source] --> B[Template Parser]
C[Variables] --> B
D[Context Data] --> B
end
subgraph "Processing Engine"
B --> E[Template Compiler]
E --> F[Logic Processor]
F --> G[Conditional Renderer]
end
subgraph "Output Generation"
G --> H[Format Renderer]
H --> I[PDF Output]
H --> J[HTML Output]
H --> K[DOCX Output]
H --> L[JSON Output]
end
subgraph "Management"
M[Template Designer] --> A
N[Variable Manager] --> C
O[Schedule Manager] --> A
P[Audit Logger] --> Q[Execution History]
end
```

**Diagram sources**
- [features/templates/TemplateEngine.tsx](file://frontend/src/features/templates/TemplateEngine.tsx)
- [features/templates/TemplateDesigner.tsx](file://frontend/src/features/templates/TemplateDesigner.tsx)
- [features/templates/VariableManager.tsx](file://frontend/src/features/templates/VariableManager.tsx)
- [features/templates/OutputRenderer.tsx](file://frontend/src/features/templates/OutputRenderer.tsx)
- [features/templates/ScheduleManager.tsx](file://frontend/src/features/templates/ScheduleManager.tsx)

**Section sources**
- [features/templates/TemplateEngine.tsx](file://frontend/src/features/templates/TemplateEngine.tsx)
- [features/templates/TemplateDesigner.tsx](file://frontend/src/features/templates/TemplateDesigner.tsx)
- [features/templates/VariableManager.tsx](file://frontend/src/features/templates/VariableManager.tsx)
- [features/templates/OutputRenderer.tsx](file://frontend/src/features/templates/OutputRenderer.tsx)
- [features/templates/ScheduleManager.tsx](file://frontend/src/features/templates/ScheduleManager.tsx)
- [hooks/useTemplates.ts](file://frontend/src/hooks/useTemplates.ts)
- [stores/templateStore.ts](file://frontend/src/stores/templateStore.ts)
- [lib/templateEngine.ts](file://frontend/src/lib/templateEngine.ts)

## Consolidated Navigation Structure

### Overview
The navigation system has been restructured around a consolidated Organisation menu that groups related functionality and provides intuitive access to all organizational management features. The new structure emphasizes workflow-based navigation rather than purely functional grouping.

### Navigation Architecture
- **Organisation Menu**: Primary navigation hub for all organizational features
- **Sub-menus**: Contextual menus based on selected organizational units
- **Quick Access**: Frequently used actions accessible from any page
- **Breadcrumb Navigation**: Hierarchical path tracking and quick navigation
- **Search Integration**: Global search across all organizational data

### Menu Structure
- **People Management**: Staff, students, parents, and contacts
- **Academic Structure**: Classes, subjects, schedules, and curriculum
- **Administrative Tools**: Reports, templates, and system configuration
- **Communication Hub**: Messaging, announcements, and notifications
- **Analytics Dashboard**: Performance metrics and reporting tools

```mermaid
graph TB
subgraph "Main Navigation"
A[Organisation Menu] --> B[People Management]
A --> C[Academic Structure]
A --> D[Administrative Tools]
A --> E[Communication Hub]
A --> F[Analytics Dashboard]
end
subgraph "People Management"
B --> B1[Staff Directory]
B --> B2[Student Records]
B --> B3[Parent Portal]
B --> B4[Contact Management]
end
subgraph "Academic Structure"
C --> C1[Class Management]
C --> C2[Subject Catalog]
C --> C3[Schedule Builder]
C --> C4[Curriculum Planning]
end
subgraph "Administrative Tools"
D --> D1[Report Generator]
D --> D2[Template Library]
D --> D3[System Config]
D --> D4[Data Import/Export]
end
subgraph "Communication Hub"
E --> E1[Messaging System]
E --> E2[Announcements]
E --> E3[Notifications]
E --> E4[Email Templates]
end
subgraph "Analytics Dashboard"
F --> F1[Performance Metrics]
F --> F2[Statistical Reports]
F --> F3[Budget Analysis]
F --> F4[Resource Utilization]
end
```

**Section sources**
- [routes/organisationRoutes.ts](file://frontend/src/routes/organisationRoutes.ts)
- [components/navigation/OrganisationMenu.tsx](file://frontend/src/components/navigation/OrganisationMenu.tsx)
- [components/navigation/BreadcrumbNav.tsx](file://frontend/src/components/navigation/BreadcrumbNav.tsx)
- [hooks/useNavigation.ts](file://frontend/src/hooks/useNavigation.ts)

## Detailed Component Analysis

### Shared UI Components
- DataTable: A flexible table component that supports pagination, sorting, filtering, and column configuration. It composes smaller elements and relies on hooks for data management.
- Card: A layout container with header, body, and footer slots. Used to group related content consistently.
- Button: A styled interactive element with variants and states. Encapsulates accessibility attributes and event forwarding.
- **Enhanced**: OrganisationalChart: Advanced visualization component with improved drag-and-drop functionality and redesigned node components for better user experience.
- **New**: ChartNode: Redesigned node component with enhanced visual feedback, improved interaction states, and better performance characteristics.
- **New**: ChartConnector: Enhanced connector system with animated transitions and customizable styling options.
- **New**: TemplateGenerator: Component for creating and managing dynamic templates with variable substitution.

```mermaid
classDiagram
class DataTable {
+props columns
+props data
+props pagination
+events onSort
+events onFilter
+events onPageChange
}
class Card {
+props title
+props variant
+slots header
+slots body
+slots footer
}
class Button {
+props variant
+props size
+props disabled
+events onClick
}
class OrganisationalChart {
+props data
+props mode
+props config
+events onNodeClick
+events onNodeDrag
+events onNodeDrop
+events onModeChange
+methods zoomIn()
+methods zoomOut()
+methods resetView()
+methods enableDrag()
+methods disableDrag()
}
class ChartNode {
+props id
+props label
+props type
+props position
+props metadata
+props draggable
+events onClick
+events onDragStart
+events onDragEnd
+events onContextMenu
+methods updatePosition()
+methods setVisualState()
}
class ChartConnector {
+props source
+props target
+props style
+props animated
+events onClick
+methods animateTransition()
+methods updatePath()
}
class TemplateGenerator {
+props templateId
+props variables
+props outputFormat
+events onGenerate
+events onSave
+events onPreview
+methods compile()
+methods validate()
+methods export()
}
DataTable --> Button : "uses"
Card --> Button : "uses"
OrganisationalChart --> ChartNode : "renders"
OrganisationalChart --> ChartConnector : "connects"
OrganisationalChart --> Card : "uses"
TemplateGenerator --> Card : "uses"
ChartNode --> ChartConnector : "connected by"
```

**Diagram sources**
- [components/ui/DataTable.tsx](file://frontend/src/components/ui/DataTable.tsx)
- [components/ui/Card.tsx](file://frontend/src/components/ui/Card.tsx)
- [components/ui/Button.tsx](file://frontend/src/components/ui/Button.tsx)
- [components/ui/OrganisationalChart.tsx](file://frontend/src/components/ui/OrganisationalChart.tsx)
- [components/ui/ChartNode.tsx](file://frontend/src/components/ui/ChartNode.tsx)
- [components/ui/ChartConnector.tsx](file://frontend/src/components/ui/ChartConnector.tsx)
- [components/ui/TemplateGenerator.tsx](file://frontend/src/components/ui/TemplateGenerator.tsx)

**Section sources**
- [components/ui/DataTable.tsx](file://frontend/src/components/ui/DataTable.tsx)
- [components/ui/Card.tsx](file://frontend/src/components/ui/Card.tsx)
- [components/ui/Button.tsx](file://frontend/src/components/ui/Button.tsx)
- [components/ui/index.ts](file://frontend/src/components/ui/index.ts)

### Providers System
- Authentication Provider: Supplies user identity, permissions, and login/logout actions. Protects routes and exposes auth state to consumers.
- Dashboard Provider: Manages dashboard-specific settings such as layout preferences, active modules, and cached configurations.
- **Enhanced**: OrganisationalChart Provider: Manages chart state, visualization modes, and user interactions with improved drag-and-drop support.
- **New**: Template Provider: Handles template lifecycle, variable management, and generation workflows.

```mermaid
sequenceDiagram
participant App as "App.tsx"
participant AuthP as "AuthProvider.tsx"
participant DashP as "DashboardProvider.tsx"
participant ChartP as "OrganisationalChartProvider.tsx"
participant TemplateP as "TemplateProvider.tsx"
participant Route as "Route"
participant Consumer as "Component"
App->>AuthP : "Wrap routes"
App->>DashP : "Wrap dashboard routes"
App->>ChartP : "Wrap chart routes"
App->>TemplateP : "Wrap template routes"
AuthP-->>Consumer : "Provide auth state/actions"
DashP-->>Consumer : "Provide dashboard config"
ChartP-->>Consumer : "Provide chart state/modes"
TemplateP-->>Consumer : "Provide template data/actions"
Route->>Consumer : "Render feature page"
```

**Diagram sources**
- [App.tsx](file://frontend/src/App.tsx)
- [features/auth/AuthProvider.tsx](file://frontend/src/features/auth/AuthProvider.tsx)
- [features/dashboard/DashboardProvider.tsx](file://frontend/src/features/dashboard/DashboardProvider.tsx)
- [features/organisation/OrganisationalChartProvider.tsx](file://frontend/src/features/organisation/OrganisationalChartProvider.tsx)
- [features/templates/TemplateProvider.tsx](file://frontend/src/features/templates/TemplateProvider.tsx)

**Section sources**
- [features/auth/AuthProvider.tsx](file://frontend/src/features/auth/AuthProvider.tsx)
- [features/dashboard/DashboardProvider.tsx](file://frontend/src/features/dashboard/DashboardProvider.tsx)
- [features/organisation/OrganisationalChartProvider.tsx](file://frontend/src/features/organisation/OrganisationalChartProvider.tsx)
- [features/templates/TemplateProvider.tsx](file://frontend/src/features/templates/TemplateProvider.tsx)

### Hooks and Data Flow
- useAuth: Encapsulates authentication logic, exposing user info, roles/permissions, and actions like login and logout.
- usePagination: Centralizes pagination state and handlers, commonly consumed by DataTable.
- **Enhanced**: useOrganisationalChart: Manages chart state, node interactions, and visualization modes with improved drag-and-drop handling.
- **New**: useTemplates: Handles template compilation, variable substitution, and output generation.

```mermaid
flowchart TD
Start(["Component renders"]) --> UseAuth["Call useAuth()"]
UseAuth --> HasUser{"Has user?"}
HasUser --> |No| Redirect["Redirect to login"]
HasUser --> |Yes| UsePagination["Call usePagination()"]
UsePagination --> FetchData["Fetch data via lib/api.ts"]
FetchData --> UpdateState["Update pagination state"]
UpdateState --> RenderTable["Render DataTable"]
Start --> UseChart["Call useOrganisationalChart()"]
UseChart --> LoadChartData["Load chart data"]
LoadChartData --> InitializeChart["Initialize chart engine"]
InitializeChart --> SetupInteractions["Setup drag-and-drop interactions"]
SetupInteractions --> RenderChart["Render OrganisationalChart"]
Start --> UseTemplates["Call useTemplates()"]
UseTemplates --> LoadTemplates["Load template library"]
LoadTemplates --> CompileTemplate["Compile selected template"]
CompileTemplate --> GenerateOutput["Generate output"]
GenerateOutput --> RenderTemplate["Render TemplateGenerator"]
RenderTable --> End(["Component ready"])
RenderChart --> End
RenderTemplate --> End
```

**Diagram sources**
- [hooks/useAuth.ts](file://frontend/src/hooks/useAuth.ts)
- [hooks/usePagination.ts](file://frontend/src/hooks/usePagination.ts)
- [hooks/useOrganisationalChart.ts](file://frontend/src/hooks/useOrganisationalChart.ts)
- [hooks/useTemplates.ts](file://frontend/src/hooks/useTemplates.ts)
- [lib/api.ts](file://frontend/src/lib/api.ts)

**Section sources**
- [hooks/useAuth.ts](file://frontend/src/hooks/useAuth.ts)
- [hooks/usePagination.ts](file://frontend/src/hooks/usePagination.ts)
- [hooks/useOrganisationalChart.ts](file://frontend/src/hooks/useOrganisationalChart.ts)
- [hooks/useTemplates.ts](file://frontend/src/hooks/useTemplates.ts)
- [lib/api.ts](file://frontend/src/lib/api.ts)

### Stores and Global State
- appStore: A lightweight store for cross-cutting application state (e.g., theme, language, notifications). Components subscribe selectively to avoid unnecessary re-renders.
- **Enhanced**: chartStore: Manages complex organizational chart state including node positions, connections, viewport settings, and user interactions with improved performance.
- **New**: templateStore: Handles template definitions, variable scopes, generation history, and output caching.

```mermaid
graph LR
Store["appStore.ts"] --> Provider["Providers / Context"]
ChartStore["chartStore.ts"] --> ChartProvider["Chart Provider"]
TemplateStore["templateStore.ts"] --> TemplateProvider["Template Provider"]
Provider --> Components["Feature Components"]
ChartProvider --> ChartComponents["Chart Components"]
TemplateProvider --> TemplateComponents["Template Components"]
Components --> Store
ChartComponents --> ChartStore
TemplateComponents --> TemplateStore
```

**Diagram sources**
- [stores/appStore.ts](file://frontend/src/stores/appStore.ts)
- [stores/chartStore.ts](file://frontend/src/stores/chartStore.ts)
- [stores/templateStore.ts](file://frontend/src/stores/templateStore.ts)

**Section sources**
- [stores/appStore.ts](file://frontend/src/stores/appStore.ts)
- [stores/chartStore.ts](file://frontend/src/stores/chartStore.ts)
- [stores/templateStore.ts](file://frontend/src/stores/templateStore.ts)

## Dependency Analysis
- Entry points depend on routing and providers.
- Features depend on shared UI and hooks.
- UI components depend on hooks and libraries but not on feature logic.
- Hooks depend on the API library and stores.
- **Enhanced**: Specialized engines (chartEngine, templateEngine, dragDropEngine) provide core functionality for complex features with improved performance.
- **New**: Dedicated stores manage complex state for visualization and template systems with better separation of concerns.

```mermaid
graph TB
main_tsx["main.tsx"] --> app_tsx["App.tsx"]
app_tsx --> route_tree["routeTree.gen.ts"]
route_tree --> auth_provider["AuthProvider.tsx"]
route_tree --> dashboard_provider["DashboardProvider.tsx"]
route_tree --> org_chart_provider["OrganisationalChartProvider.tsx"]
route_tree --> template_provider["TemplateProvider.tsx"]
auth_provider --> use_auth["useAuth.ts"]
dashboard_provider --> use_pagination["usePagination.ts"]
org_chart_provider --> use_chart["useOrganisationalChart.ts"]
template_provider --> use_templates["useTemplates.ts"]
use_pagination --> api_lib["lib/api.ts"]
use_chart --> chart_engine["chartEngine.ts"]
use_chart --> drag_drop["dragDropEngine.ts"]
use_templates --> template_engine["templateEngine.ts"]
ui_datatable["DataTable.tsx"] --> use_pagination
ui_card["Card.tsx"] --> ui_button["Button.tsx"]
ui_chart["OrganisationalChart.tsx"] --> use_chart
ui_chart --> drag_drop
ui_node["ChartNode.tsx"] --> drag_drop
ui_connector["ChartConnector.tsx"] --> drag_drop
ui_template["TemplateGenerator.tsx"] --> use_templates
app_tsx --> app_store["appStore.ts"]
org_chart_provider --> chart_store["chartStore.ts"]
template_provider --> template_store["template_store.ts"]
```

**Diagram sources**
- [main.tsx](file://frontend/src/main.tsx)
- [App.tsx](file://frontend/src/App.tsx)
- [routeTree.gen.ts](file://frontend/src/routeTree.gen.ts)
- [features/auth/AuthProvider.tsx](file://frontend/src/features/auth/AuthProvider.tsx)
- [features/dashboard/DashboardProvider.tsx](file://frontend/src/features/dashboard/DashboardProvider.tsx)
- [features/organisation/OrganisationalChartProvider.tsx](file://frontend/src/features/organisation/OrganisationalChartProvider.tsx)
- [features/templates/TemplateProvider.tsx](file://frontend/src/features/templates/TemplateProvider.tsx)
- [hooks/useAuth.ts](file://frontend/src/hooks/useAuth.ts)
- [hooks/usePagination.ts](file://frontend/src/hooks/usePagination.ts)
- [hooks/useOrganisationalChart.ts](file://frontend/src/hooks/useOrganisationalChart.ts)
- [hooks/useTemplates.ts](file://frontend/src/hooks/useTemplates.ts)
- [components/ui/DataTable.tsx](file://frontend/src/components/ui/DataTable.tsx)
- [components/ui/Card.tsx](file://frontend/src/components/ui/Card.tsx)
- [components/ui/Button.tsx](file://frontend/src/components/ui/Button.tsx)
- [components/ui/OrganisationalChart.tsx](file://frontend/src/components/ui/OrganisationalChart.tsx)
- [components/ui/ChartNode.tsx](file://frontend/src/components/ui/ChartNode.tsx)
- [components/ui/ChartConnector.tsx](file://frontend/src/components/ui/ChartConnector.tsx)
- [components/ui/TemplateGenerator.tsx](file://frontend/src/components/ui/TemplateGenerator.tsx)
- [lib/api.ts](file://frontend/src/lib/api.ts)
- [lib/chartEngine.ts](file://frontend/src/lib/chartEngine.ts)
- [lib/dragDropEngine.ts](file://frontend/src/lib/dragDropEngine.ts)
- [lib/templateEngine.ts](file://frontend/src/lib/templateEngine.ts)
- [stores/appStore.ts](file://frontend/src/stores/appStore.ts)
- [stores/chartStore.ts](file://frontend/src/stores/chartStore.ts)
- [stores/templateStore.ts](file://frontend/src/stores/templateStore.ts)

**Section sources**
- [main.tsx](file://frontend/src/main.tsx)
- [App.tsx](file://frontend/src/App.tsx)
- [routeTree.gen.ts](file://frontend/src/routeTree.gen.ts)
- [components/ui/index.ts](file://frontend/src/components/ui/index.ts)

## Performance Considerations
- Memoization: Wrap expensive computations and derived values with memoization hooks to prevent unnecessary recalculations.
- Selective subscriptions: Subscribe only to needed parts of global state to minimize re-renders.
- Lazy loading: Load heavy feature routes and large components lazily to reduce initial bundle size.
- Virtualization: For large lists or tables, consider virtualized rendering to improve scroll performance.
- Stable references: Stabilize callback and object props to help child components skip updates when using memoization.
- Debounce inputs: Debounce search/filter inputs to reduce network requests and processing overhead.
- Image/media optimization: Use responsive images and lazy loading for media assets.
- **Enhanced**: Canvas rendering: Use canvas-based rendering for complex charts to improve performance with large datasets and smooth animations.
- **Enhanced**: Web Workers: Offload heavy template processing and chart calculations to background threads to prevent UI blocking.
- **Enhanced**: Memory management: Implement proper cleanup for chart instances, drag operations, and template compilations to prevent memory leaks.
- **New**: Optimized drag-and-drop: Efficient collision detection and positioning algorithms for smooth user interactions.
- **New**: Batch updates: Group multiple state updates to minimize re-renders during complex operations.

## Troubleshooting Guide
- Authentication issues: Verify provider wrapping around protected routes and ensure tokens are correctly handled in hooks.
- Pagination problems: Check hook state initialization and API response shape; validate page size and total counts.
- UI inconsistencies: Ensure shared UI components receive expected props and that styles/themes are applied consistently.
- Global state conflicts: Confirm that stores are updated immutably and that selectors are precise to avoid cascading re-renders.
- Network errors: Centralize error handling in the API library and surface user-friendly messages via providers or toast utilities.
- **Enhanced**: Chart rendering issues: Verify data structure compatibility with chart engine and check for circular dependencies in node relationships. Monitor memory usage for large datasets.
- **Enhanced**: Drag-and-drop problems: Ensure proper event handling and check for conflicting mouse/touch events. Verify coordinate calculations and boundary constraints.
- **Enhanced**: Node component issues: Validate node properties and check for proper cleanup of event listeners. Ensure visual states are properly managed.
- **New**: Template compilation errors: Validate template syntax and variable references; check for undefined variables in context.
- **New**: Navigation problems: Ensure route guards are properly configured and permission checks are working correctly.

## Conclusion
The architecture emphasizes feature-based organization, reusable UI primitives, and clear separation of concerns through providers, hooks, and stores. The recent enhancements have significantly improved the organizational chart visualization system with advanced drag-and-drop functionality, redesigned node components, and optimized performance. These improvements provide powerful tools for complex organizational data management while maintaining scalability, testability, and performance. The enhanced system supports sophisticated user interactions, real-time collaboration, and efficient rendering of large datasets, making it suitable for enterprise-scale applications requiring complex hierarchical data visualization and manipulation. By adhering to composition patterns, stable prop interfaces, and careful state management, teams can maintain a robust and evolving codebase that supports increasingly sophisticated business requirements.