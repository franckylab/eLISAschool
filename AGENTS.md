...
- **Couche d'intégration** `frontend/src/components/layout/fond-anime.tsx` : 3 couches — (1) dégradé général OPAQUE (`FOND_DEGRADE_LIGHT` : #fff→#e6f0ff→#fff3d6 135° / `FOND_DEGRADE_DARK` : #0e2418→#14264d→#3f3414→#1d2438), (2) quadrillage « cahier » statique 100% CSS (`FOND_GRILLE_*` : cellule 28px, lignes + points embossés 3 couches aux intersections via 16 background-image sur 1 div — zéro DOM/JS/repaint par frame ; variantes `FOND_GRILLE_LIGHT` (#93c5fd) / `FOND_GRILLE_DARK` (#22c55e — vert eLISAschool)), (3) lettres Text Wave (opacité 0.4 via `FOND_ANIME_*`, `backgroundColor="transparent"`). Résolution thème au runtime (`getComputedStyle` + MutationObserver `data-theme` + `normaliserCouleurHex` depuis `@/lib/export`), fallback hex canon (dominant #22c55e / accent #007bff / secondary #f59e0b). `prefers-reduced-motion` → `speed={0}` (grille figée, rAF réduit à du recalc sans changement visuel). Wrapper `fixed inset-0 -z-20`, `aria-hidden`, `pointer-events: none`. Vérif CDP 2026-07-31 : 162 tuiles, `--t` animé (rAF), bascule light/dark instantanée via MutationObserver, diff pixel quadrillage light 2.05 (2.9%) / dark 1.25 (2.4%). Dégradé dark sans bleu : #0e2418→#14532d (vert)→#3f3414→#1d2438 (grill-me 2026-07-31).
